...
//UI testing and end to end testing
class GetNotes(
        ...
        return repository.getNotes().map { notes ->
            when(noteOrder.ordertype) {
                ...
                is Ordertype.Descending -> {
                    when(noteOrder) {
                        is Note_order.Title -> notes.sortedByDescending { it.title.lowercase() }
                        is Note_order.Date -> notes.sortedByDescending { it.timeStamp }
                        is Note_order.Color -> notes.sortedByDescending { it.color }
                    }
                }
            }
        }
